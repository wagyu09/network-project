...
2.  **중심성 분석 (Centrality Analysis)**
    ...
    - 각 서브그래프 내에서 **지역 중심성(Local Degree Centrality)** 을 별도로 계산합니다. 이는 각 주식이 자신이 속한 커뮤니티 내에서 얼마나 핵심적인 역할을 하는지를 나타냅니다.
...
- **`final_summary_report.csv`**: 이 파일이 최종 결론을 담고 있습니다. 각 전략 포트폴리오의 전체 기간 평균 성과 지표와 함께 아래와 같은 통계적 분석 결과를 제공합니다.
  ...
  - **`p-value (지표)`**: `Top %` 값의 평균이 단순한 우연(50%)보다 통계적으로 유의미하게 우수한지를 나타내는 값입니다. 롤링 윈도우로 인한 시계열 자기상관을 보정한 **Newey-West p-value**가 사용되었습니다. 일반적으로 p-value가 0.05 미만일 경우, 해당 전략의 성과는 우연이 아닌 유의미한 결과로 해석합니다.
  - **`random_avg` 행**: 전체 기간 동안 생성된 모든 무작위 포트폴리오들의 성과 평균입니다. 전략 포트폴리오의 성과를 직접 비교하는 기준점(baseline) 역할을 합니다.
