# 주식 시장 네트워크 분석 기반 포트폴리오 전략 백테스팅 프로젝트

## 1. 프로젝트 개요 (Introduction)

이 프로젝트는 **주식 시장을 하나의 거대한 네트워크(연결망)로 간주**하고, 그 안에서 특정 종목이 차지하는 구조적 위치(중심성)가 투자 성과에 어떤 영향을 미치는지 분석하는 것을 목표로 합니다.

단순히 개별 주식의 가격 변동만 보는 것을 넘어, **주식들 간의 숨겨진 상호 연관성**을 네트워크 형태로 시각화하고 분석함으로써 새로운 투자 전략의 유효성을 검증합니다.

핵심 아이디어는 다음과 같습니다.
> 시장 전체의 움직임을 제외했을 때, 주식들 사이에 나타나는 순수한 연관성을 바탕으로 네트워크를 구축하고, 이 네트워크의 중심에 있는 주식과 외곽에 있는 주식으로 포트폴리오를 구성하면 어떤 성과 차이가 나타날까?
...
1.  **커뮤니티 탐지 (Community Detection)**
    ...
    - 이는 주로 비슷한 산업군이나 가치 사슬로 묶인 기업들로 구성되는 경향이 있습니다.

2.  **중심성 분석 (Centrality Analysis)**
    - 각 주식이 네트워크(또는 커뮤니티) 내에서 얼마나 많은 연결을 가지며 중심적인 역할을 하는지를 나타내는 **연결 중심성(Degree Centrality)**을 계산합니다.
...
- **롤링 윈도우 방식**: 분석 기간을 분기별로 나누어 진행합니다.
  ...
  - 구성된 포트폴리오를 **다음 2분기 동안** 운용했을 때의 성과를 측정합니다.
