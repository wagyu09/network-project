...
2.  **중심성 분석 (Centrality Analysis)**
    - **전체 네트워크**에 포함된 모든 주식에 대해 개별적으로 **Degree Centrality**를 계산합니다. 이는 각 주식이 시장 전체에서 얼마나 많은 다른 주식과 직접적으로 연결되어 있는지를 나타내는 지표입니다.
    - 이후, 각 커뮤티니별로 그룹화된 주식들 중에서, 미리 계산된 **전체 네트워크 기준 중심성 점수**가 가장 높은 주식과 가장 낮은 주식을 식별합니다.
...
- **`final_summary_report.csv`**: 이 파일이 최종 결론을 담고 있습니다. 각 전략 포트폴리오의 평균 성과 지표와 함께 **`Avg_Random_Outperformance_Rate`** 라는 중요한 지표를 제공합니다.
  - 이 값은 저희의 전략 포트폴리오가 무작위 포트폴리오보다 성과가 나빴던 비율의 평균을 의미합니다. 예를 들어, `Avg_Random_Outperformance_Rate (Return)` 값이 0.3이라면, 전체 기간 동안 저희 전략의 수익률이 무작위 포트폴리오 중 상위 30% 수준에 위치했다는 의미로 해석할 수 있습니다. 즉, 이 값이 낮을수록 전략의 우수성이 높다고 볼 수 있습니다.
